                ...
                <div class="settled-singer">
                    ...
                    <div class="list">
                        <div class="card" v-for="(item, index) in singers" :key="item.id">
                          <router-link :to="`/singerInfo/${item.id}`" @click.native="setSingerDetail(item.id)">
                             <div class="pic">
...
export default {
  ...
  computed: {
    ...mapState(["cat", "currentPage", "radioValue"]),
  },
  async mounted() {
  ...
  methods: {
    changeCat(item) {
      ...
      this.$store.dispatch("setSheets", this.currentPage, this.radioValue);
    },
    setSingerDetail(id) {

...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async setSheets(context, page, order, cat) {
      const { data: res } = await axios.get(
        `top/playlist?cat=${cat}&order=${order}&offset=${page}&limit=35`
      );
      context.commit("setSheets", res);
    },
    async setOrder(context, order) {
      const { data: res } = await axios.get(
        `top/playlist?cat=${context.state.cat}&order=${order}&offset=1&limit=35`
      );
      context.commit("setSheets", res);
    },
  },

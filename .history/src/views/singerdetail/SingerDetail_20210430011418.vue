...
export default {
  ...
  computed: {
    ...
    ...mapState(["singerDetail", "ready", "playState", "playList"]),
  },
  created() {
  ...
  methods: {
    ...
    async songListPlay() {
      this.$store.commit("playModeChangeLoop");
      this.$store.commit("setPlayList", this.sheetDetail.tracks);
      this.$store.commit("setCurrentIdxFirst");
      this.$store.commit("setCurrentSongId", this.sheetDetail.tracks[0].id);
      this.$store.commit("setCurrentSong", this.sheetDetail.tracks[0]);
      const currentId = this.sheetDetail.tracks[0].id;

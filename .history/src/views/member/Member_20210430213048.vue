...
export default {
  ...
  computed: {
    ...mapState(["userDetail", "singRecord", "currentSongId"]),
  },
  mounted() {
    this.setUserRecord(this.$route.params.id);
    this.setUserSheets(this.$route.params.id);
  },
  methods: {
    setUserRecord(id) {
      this.$store.dispatch("setUserRecord", id);
    },
    setUserSheets(id) {
      this.$store.dispatch("setUserSheets", id);
    },
    onPlayClick(item) {

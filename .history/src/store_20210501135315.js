...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    setHasTokenFalse(state) {
      state.hasToken = false;
    },
    setUserRecord(state, data) {
      state.userRecord = data;
    },
    changeSingRecordWeek(state) {
      state.singRecord = 1;
    },
    changeSingRecordAll(state) {
      state.singRecord = 0;
    },
    setSimiSinger(state, artists) {
      state.simiSinger = artists;
    },
  },
  actions: {
    ...
    async userLogin(context, values) {
      ...
      if (phone.trim() !== "" && password.phone !== "") {
        ...
        if (res.msg === "密码错误") {
          ...
          this.$message.success("用户成功登录");
          context.commit("setUserDetail", user);
          context.commit("setHasTokenTrue");
          context.commit("setIsModalVisibleFalse");
        }
        if (remember) {
          localStorage.setItem(
            "wyRememberLogin",
            JSON.stringify(codeJson(values))
          );
        } else {
    ...
    async logout(context) {
      ...
      if (res.statusText === "OK") {
        context.commit("setHasTokenFalse");
        this.$message.success("用户成功退出");
      }
    },
    async setUserRecord(context, id) {
      const { data: res } = await axios.get(
        `user/record?uid=${id}&type=${context.state.singRecord}`
      );
      context.commit("setUserRecord", res.allData ? res.allData : res.weekData);
    },
    async setUserSheets(context, id) {
      const { data: res } = await axios.get(`user/playlist?uid=${id}`);
      console.log(res);
    },
    async setSimiSinger(context, id) {
      const { data: res } = await axios.get(`simi/artist?id=${id}`);
      context.commit("setSimiSinger", res.artists);
    },
  },
  modules: {},
});

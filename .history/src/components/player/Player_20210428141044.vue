  ...
  <div class="m-player">
    ...
    <div class="container">
      <div class="wrap">
        ...
        <div class="ctrl">
          <i class="volume" title="音量" @click="!showVolume"></i>
          <i class="loop"></i>
          <p class="open">
            <span></span>
          </p>
          <div :class="[showVolume ? 'control-vol' : 'hide']">
            <a-slider
              style="margin: 0; padding: 0"
              vertical
...
export default {
  ...
  async updated() {
    const { playList, currentIdx, ready, playState } = this.$store.state;
  ...
  methods: {
    ...
    onPercentChange(newValue) {
      ...
      this.$store.commit("setCurrentTimeChanged", newCurrent);
      this.$store.commit("setCurrentPercentChanged", newPercent);
    },

      ...
      <div class="wrap">
        ...
        <div class="ctrl">
          ...
          <div class="control-vol">
            <a-slider style="margin: 0; padding: 0" vertical />
          </div>
        </div>
      </div>
    </div>
    <audio ref="audioRef" src=""></audio>
    <span>{{ $store.state.songUrl }}</span>
  </div>
</template>
<script>
export default {
  ...
  async created() {
    ...
    if (playList.length && currentIdx !== -1) {
      if (ready) {
        ...
        const resultUrl = await this.$axios.get(`song/url?id=${currentId}`);
        this.$store.state.songUrl = resultUrl.data.data[0].url;
        if (playState) {

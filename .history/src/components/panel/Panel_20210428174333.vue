...
export default {
  ...
  updated() {
    const { currentLyric, currentLineNum, currentTime } = this.$store.state;
    if (this.playList.length) {
      if (playState && currentLyric.length) {
        const play = () => {
          this.$store.commit(findCurNum(Math.floor(currentTime) * 1000));
        };
        const findCurNum = (time) => {
          const index = currentLyric.findIndex((item) => item <= item.time);

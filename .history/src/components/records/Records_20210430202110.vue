...
export default {
  ...
  computed: {
    listenChange() {
      const { singRecord, userRecord } = this;
      return { singRecord, userRecord };
    },
    ...mapState(["singRecord", "userRecord"]),
  },
  watch: {
    listenChange() {
      console.log(this.singRecord);
      console.log(this.userRecord);
      if (this.userRecord.length) {
        this.userRecord.forEach((item, idx) => {
          ...
          this.dataSource.push({
            ...
            playIcon: null,
            song: item.song,
            time: this.$transformTime(item.song.dt / 1000),
